...
public class MainActivity extends AppCompatActivity {
    ...
    private class SpaceTask extends AsyncTask<Void, Void, Void> {
        ...
        @Override
        protected void onPostExecute(Void voidk) {
            super.onPostExecute(voidk);
            ...
            if(spaceResponse.media_type.equals("image")){
                Picasso.get().load(spaceResponse.url).placeholder(R.drawable.spacemen).into(spaceImage);
            }else {
                spaceImage.setImageResource(R.drawable.spacemen);
                describText.append("\n" + spaceResponse.url);
                Toast.makeText(getApplicationContext(), "Сегодня в программе видео", Toast.LENGTH_SHORT);
            }
